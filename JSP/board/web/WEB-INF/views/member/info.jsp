...
<head>
    ...
    <script>
        ...
        function joinMember() {
            ...
            if (addressFlag && (zipcode.val() == '' || address.val() == '' || detailAddress.val() == '')) {
...
<div>
    휴대전화번호 : <%=dto.getMobileNo()%>
</div>
<div>
    이메일 : <%=dto.getEmail()%>
</div>
<div>
    주소 : (<%=dto.getZipcode()%>) <%=dto.getAddress()%>
...
<form action="/member/join.do" method="post" id="joinForm">
    <div>
        ...
        <input type="text" name="loginId" maxlength="15" id="loginId"
               placeholder="4~15자의 영문소문자, 숫자로 입력해주세요."
               oninput="initCheck()">
    ...
    <div>
        이름
        <input type="text" name="name" maxlength="10" id="name" placeholder="2~10자의 한글로 입력해 주세요.">
    </div>
    <div>
        생년월일
        <input type="text" name="birthday" maxlength="8" id="birthday" placeholder="ex) 19991230">
    </div>
    <div>
        휴대전화번호
        <input type="text" name="mobileNo" maxlength="11" id="mobileNo" placeholder="-를 제외한 숫자로 입력해 주세요">
    </div>
    <div>
        이메일
        <input type="email" name="email" maxlength="50" id="email" placeholder="">
    </div>
    <div>
        주소
...
<%--table>tr*3>td*7--%>
<%--input*3 하고 tab누르면 자동으로 코드 쳐줌--%>
<%--readonly : 사용자가 직접 입력할 수는 없게 됨--%>
<%--onkey--%>
<%--oninput--%>
<%--onkey--%>